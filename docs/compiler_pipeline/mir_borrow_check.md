...
### MIR 中的内存管理指令：不是"识别回收点"

初学者常有的误解：MIR 是不是像 GC 那样"识别"什么时候该回收内存？**不是**。

GC（Java/Go）在**运行时**扫描堆内存，推测哪些对象不再被引用，然后回收。这是**运行时识别回收点**。

Rust 在**编译期**就确定了每个值的释放时机——由所有权规则决定。MIR 中的内存管理指令只是把这个**已确定的释放时机**表达在控制流图上：

| MIR 指令 | 含义 | 与 GC 的区别 |
|---------|------|------------|
| `StorageLive(_1)` | 标记变量 `_1` 开始存活 | 编译期确定，非运行时分配 |
| `StorageDead(_1)` | 标记变量 `_1` 结束存活 | 编译期确定，非运行时回收 |
| `Drop { place: _1 }` | 调用 `_1` 的析构函数 | 编译期插入，确定性调用；GC 是运行时推测式扫描 |

**关键洞察**：Rust 不需要 MIR 去"识别"释放时机——所有权系统在编译期就已经决定了。MIR 只是把这个决定记录下来，变成可验证、可优化的中间表示。
